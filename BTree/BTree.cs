...
namespace BTree
{
    public partial class BTreeForm : Form
    {
        ...
        private void Exercise3__Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < 15; ++i)
            {
                node = new BTree(uppercaseLetters[random.Next(uppercaseLetters.Length)].ToString(), root);
                if (i == 0)
                {
                    root = node;
                }
        ...
        private void Exercise5__Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < 26; ++i)
            {
                node = new BTree(uppercaseLetters[i].ToString(), root);
                if (i == 1)
                {
                    root = node;
                }
        ...
        private void Exercise6__Click(object sender, EventArgs e)
        {
            ...
            // Your code here
            Person me = new Person("Me", 18);
            Person brother = new Person("Brother", 25);
            Person mom = new Person("Mom", 50);

            Person aunt = new Person("Aunt", 54);
            Person aunt1 = new Person("Aunt1", 48);
            Person aunt2 = new Person("Aunt2", 52);

            Person uncle = new Person("Uncle", 51);
            Person uncle1 = new Person("Uncle1", 52);
            Person uncle2 = new Person("Uncle2", 53);
            Person uncle3 = new Person("Uncle3", 54);

            Person cousin = new Person("Cousin", 13);
            Person cousin1 = new Person("Cousin1", 30);
            Person cousin2 = new Person("Cousin2", 32);
            Person cousin3 = new Person("Cousin3", 34);
            Person niece = new Person("Niece", 5);

            Person[] persons = [me, brother, mom, aunt, aunt1, aunt2, uncle, uncle1, uncle2, uncle3, cousin, cousin1, cousin2, cousin3, niece];

            for(int i = 0; i < persons.Length; i++)
            {
                if (persons[i].GetType() == typeof(Person))
                {
                    node = new BTree(persons[i], root);
                    if (i == 0)
                    {
                        root = node;
                    }
        ...
        private void Exercise7__Click(object sender, EventArgs e)
        {
            ...
            node = new BTree(new Person("Person", 29), root, false);
            root = node;
            node = new BTree(new Person("Person1", 17), root, false);
            node = new BTree(new Person("Person2", 42), root, false);
            node = new BTree(new Person("Person3", 11), root, false);
            node = new BTree(new Person("Person4", 23), root, false);
            node = new BTree(new Person("Person5", 36), root, false);
            node = new BTree(new Person("Person6", 48), root, false);
